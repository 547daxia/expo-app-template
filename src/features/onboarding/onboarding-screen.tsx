    ...
    >
      ...
      <VStack className="mx-auto w-full max-w-lg gap-8">
        <VStack className="gap-3">
          <Text {...selectableTextProps} className="text-sm font-semibold text-primary uppercase">
            Welcome
          </Text>
          <Heading
            {...selectableTextProps}
            {...textTestIdProps('onboarding-title')}
            size="3xl"
          >
            {Env.EXPO_PUBLIC_NAME}
          </Heading>
          <Text {...selectableTextProps} className="text-lg/7 text-muted-foreground">
        ...
        <Card className="gap-4 rounded-2xl border border-border bg-card p-5">
          {FEATURES.map(feature => (
            <Text key={feature} {...selectableTextProps} className="text-base">
              ✓
              {' '}
              {feature}
            </Text>
