...
>(({ className, children, ...props }, ref) => {
  ...
  useEffect(() => {
    ...
    // @ts-expect-error - the creator context does not expose this extension point
   ...
   */
  useEffect(() => {
    if (!context || orientation !== 'horizontal' || !selectedKey)
      return;
  ...
  }, [context, selectedKey, orientation, children]);
  ...
  const nativeScrollHandler = useAnimatedScrollHandler({
    onScroll: (event) => {
      ...
      if (userOnScroll) {
        runOnJS(userOnScroll)(event as never);
      }
    },
  }, [setScrollOffset, userOnScroll]);
  ...
  const setHorizontalListRef = useCallback((node: any) => {
