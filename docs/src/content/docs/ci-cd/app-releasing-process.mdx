...
- `eas-build-preview.yml` : A workflow that is triggered whenever a new release is created on GitHub. It will build the app using EXPO EAS and based on the config it will distribute.
...
When the name of your organization is ready, set `EXPO_ACCOUNT_OWNER`, `EXPO_SLUG`, and the EAS project ID in your local or EAS environment configuration. See [First Project Setup](/getting-started/first-project-setup/) for the full ownership handoff.
...
:::caution
You need to be logged in to EAS using `pnpm dlx eas-cli@latest login` before running this command. Check [step 1 and 2 from EAS docs](https://docs.expo.dev/build/setup/) for more details.
:::
...
Never push `.env` files. Configure client-safe values such as `EXPO_PUBLIC_API_URL` in the matching EAS environment (`development`, `preview`, or `production`) and keep build-only secrets in EAS secrets. The build profiles in `eas.json` select the matching environment automatically.
...
The EAS build service reads the selected environment itself. The GitHub workflow only needs an `EXPO_TOKEN` that can access the EAS project.
...
After that, `eas-build-preview` workflow will be triggered automatically and build the app using EAS and distribute it to the QA team.
