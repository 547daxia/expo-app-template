...
- `.github/workflows/lint-ts.yml`: On PRs and new commits on `main` or `master`, run lint checks.
- `.github/workflows/test.yml`: On PRs and new commits on `main` or `master`, run unit tests.
- `.github/workflows/type-check.yml`: On PRs and new commits on `main` or `master`, run type checking.
- `.github/workflows/compress-images.yml`: On new commits to `main` or `master`, when adding images, open a pull request (PR) with the compressed images.
- `.github/workflows/expo-doctor.yml`: When `package.json` or `pnpm-lock.yaml` changes, check whether dependencies are aligned with the Expo SDK version.
