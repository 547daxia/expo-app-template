...
The Jest configuration also enforces a modest global coverage floor. Raise the thresholds in `jest.config.js` as business logic and integration coverage grow; do not lower them to accommodate untested changes.
